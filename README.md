...
| 字段 | 类型 | 说明 |
|------|------|------|
| `park_level` | string | 园区级别：`国家级` / `省级`，可为 null |
...
**已实现的扩展方向：**
- ✅ 按 31 省轮询：`"零碳园区" 招标 {province}` × 31
- ✅ 按工程内容细化：`"零碳园区" 光伏` `"零碳园区" 储能` `"零碳园区" 碳监测` 等 30+ 细化关键词
- ✅ 多通道搜索：Tavily + Serper + TalorData 三通道并行
...
| `evaluated_bids.jsonl` | 增量评估记录（129条，断点续传） |
| `bids_data.json` | 全量 JSON 数据（129条，`bid_agent.py` 导出） |
| `bids_data.xlsx` | Excel 输出（运行 `bid_agent.py` 生成，2个Sheet） |
| `数据库.json` | JSON 数据副本（122条） |
| `数据库.xlsx` | Excel 数据副本（122条） |
| `金融版.xlsx` | Excel 金融版（129条 + 统计概要 + 图表说明，3个Sheet） |
| `bids_data_embedded.js` | 前端数据文件（JS变量嵌入 `零界点.html`） |
| `零界点.html` | 零界点招标数据智能平台网页（约2MB，含完整交互功能） |
| `.env` | API Keys: TAVILY_API_KEY, DEEPSEEK_API_KEY, SERPER_API_KEY, TALORDATA_API_KEY |
